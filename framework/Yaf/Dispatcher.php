...
class Yaf_Dispatcher {
	...
	private function getAction($appDir, Yaf_Controller_Abstract $controller, $action, $module) {
		...
		if (isset($controller->actions[$action]))
		{
			...
			if (Yaf_Loader::import($actionPath))
			{
				...
				{
					throw new Yaf_Exception_LoadFailed_Action('Could not find action ' . $classname . ' in ' . $actionPath);
				}
				$object = new $classname();
				if (!($object instanceof Yaf_Action_Abstract))
				{
					throw new Yaf_Exception_TypeError('Action ' . $classname . ' must extends from Yaf_Action_Abstract');
				}
			...
			{
				throw new Yaf_Exception_LoadFailed_Action('Could not find action script ' . $actionPath);
			}
		}
		else
		{
			...
			if (!class_exists($classname, false))
			{
				if (!Yaf_Loader::getInstance()->internal_autoload($classname, $actionrDir)
				)
				{
					throw new Yaf_Exception_LoadFailed_Action('Could not find action script ' . $actionrDir);
				}
			}
			if (!class_exists($classname, false))
			{
				throw new Yaf_Exception_LoadFailed('Could not find class ' . $classname . ' in action script ' . $actionrDir);
			}
			$object = new $classname();
			if (!($object instanceof Yaf_Action_Abstract))
			{
	...
	private function prepareActionParams($request, $methodParams) {
		...
		foreach ($methodParams as $param)
		{
			...
			elseif (empty($params[$name]))
			{
				...
				throw new Yaf_Exception('Parameter: ' . $name . ' cannot be empty');
			}
			else
			{
